...
def vote_k(train_data, test_data, train_labels, test_labels, test_mode, num_demos, sub_index, max_predict_num, model_type, way_select_demo, is_model_online, test_num, combine_method, way_anno, annotate_num):
    ...
    if way_anno == "random":
        ...
        print('random', left_local_indices, right_local_indices)
        selected_indices = combineIndices(combine_method, left_local_indices, right_local_indices, left_indices, right_indices)
        ...
        left_local_indices,_ = rd("basic", left_data, left_labels, n_members=5, n_initial=1, n_queries=annotate_num // 2, measurement=measurement, estimator=RandomForestClassifier, init_indices=left_init_local_indices)
        right_local_indices,_ = rd("basic", right_data, right_labels, n_members=5, n_initial=1, n_queries=annotate_num // 2, measurement=measurement, estimator=RandomForestClassifier, init_indices=right_init_local_indices)
        print('rd_basic', left_local_indices, right_local_indices)
        selected_indices = combineIndices(combine_method, left_local_indices, right_local_indices, left_indices, right_indices)
        ...
        print('centroids', left_local_indices, right_local_indices)
        selected_indices = combineIndices(combine_method, left_local_indices, right_local_indices, left_indices, right_indices)
    ...
    for i in range(Y.shape[0]):
        ...
        # y_pred = collect_y_pred_single(example_data, example_labels, predict_data, model_type, is_model_online)
        y_pred = find_most_common(example_labels) # knn算法
        y_pred_list.extend(y_pred)
...
if __name__ == '__main__':
    ...
    combine_method = 'lasagna-left' # shuffle, lasagna-left, lasagna-right
    repeat_times = 100
    ...
    def compare_test(n_times):
        ...
        for i in tqdm(range(n_times)):
            ...
            # accuracy9, precision9, recall9, f19 = vote_k(train_data, test_data, train_labels, test_labels, test_mode, num_demos, sub_index, max_predict_num, model_type, "kate_basic", is_model_online, test_num, combine_method, "rd_basic", annotate_num)
